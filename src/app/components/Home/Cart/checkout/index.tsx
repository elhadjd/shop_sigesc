  ...
  return (
    <div className='w-full p-4'>
              ...
              <tr key={index}>
                <td className='p-4 flex items-center space-x-4'>
                  <span className=' w-[55px] rounded'>
                    <img src={`/products/img/imag1.png`} alt='' className='h-[82px] '/>
                  </span>
                ...
                <td>
                  <div className='h-full space-x-2 flex w-36 items-center'>
                    ...
                    <input type="text" value={item.quantity} placeholder='Quantidade' onChange={(e)=>item.quantity = Number(e.target.value)} onBlur={(e)=>addItem(item.produto,item.quantity)} className='w-16 text-center border outline-0 rounded'/>
                    <button type='button' onClick={()=>addItem(item.produto,1)} className='p-2 rounded text-red-700'>+</button>
                  </div>
                </td>
      ...
      <div className='flex p-4 h-20 justify-between items-center'>
        ...
        <button onClick={()=>changeStep(1)} type='button' className='bg-red-700 flex p-2 rounded text-white text-base w-64 justify-center'>Finalizar compra</button>
      </div>
    </div>
  )
}
